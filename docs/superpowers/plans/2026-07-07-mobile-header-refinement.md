...
```tsx
className =
  "flex h-10 w-10 items-center justify-center rounded-md border border-border-glass/60 bg-surface-glass/70 text-text-primary transition-all duration-300 ease-standard hover:border-border-strong hover:bg-hover-surface focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus-ring";
```
...
```tsx
className =
  "mobile:w-screen mobile:max-w-none tablet:w-[85vw] tablet:max-w-sm bg-[linear-gradient(180deg,rgb(8_17_29/0.96),rgb(16_24_39/0.94))] backdrop-blur-modal border-b border-border-glass tablet:border-b-0 tablet:border-l shadow-[0_24px_80px_rgb(0_0_0_/_0.38)]";
```
...
```tsx
<div className="w-full [&>*]:w-full">{cta}</div>
```
