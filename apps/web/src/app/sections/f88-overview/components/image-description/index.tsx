...
export default function ImageDescription() {
  const t = useTranslations("HomePage.f88Overview.imageAlt");

  return (
    <div className={clsx("order-1 grid gap-2 xl:order-2")}>
      <AppBorderRadius cornerRadius={12}>
        <Image src={F88Overview1} alt={t("first")} className={clsx("h-41.25 w-full object-cover xl:h-56")} />
      </AppBorderRadius>
      <div className={clsx("grid grid-cols-[247px_1fr] gap-2 xl:grid-cols-[280px_1fr] xl:gap-2.5")}>
        <AppBorderRadius cornerRadius={12}>
          <Image src={F88Overview2} alt={t("second")} className={clsx("h-35.75 w-full object-cover xl:h-40.75")} />
        </AppBorderRadius>
        <AppBorderRadius cornerRadius={12}>
          <Image src={F88Overview3} alt={t("third")} className={clsx("h-35.75 w-full object-cover xl:h-40.75")} />
        </AppBorderRadius>
      </div>
    </div>
  );
