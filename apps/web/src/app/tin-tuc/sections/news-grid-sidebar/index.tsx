...
function DateBadge({ day, month }: { day: string; month: string }) {
  return (
    <div className="absolute right-4 top-4 z-10 flex flex-col items-center rounded-[8px] bg-app-accent-blue px-3 py-1.5 shadow-[0px_10px_15px_-3px_rgba(0,0,0,0.1),0px_4px_6px_-4px_rgba(0,0,0,0.1)]">
      <span className="text-[18px] font-black leading-[18px] text-white">{day}</span>
      <span className="text-[12px] font-semibold text-[#bfdbfe]">{month}</span>
    </div>
  );
}
...
function CategoryTag({ label, bgColor }: { label: string; bgColor: string }) {
  return (
    <span
      className="inline-block rounded-[6px] px-2.5 py-1 text-[12px] font-bold uppercase tracking-[0.6px] text-white"
...
function NewsCard({
  ...
  return (
    <article className="flex flex-col overflow-clip rounded-[16px] border border-[#f3f4f6] bg-white shadow-[0px_4px_6px_-1px_rgba(0,0,0,0.1),0px_2px_4px_-2px_rgba(0,0,0,0.1)]">
      <div className="relative h-[192px] w-full shrink-0 overflow-clip">
        ...
        <div className="absolute bottom-[15px] left-4 z-10">
          <CategoryTag label={category} bgColor={categoryBg} />
        </div>
      </div>
      <div className="flex flex-1 flex-col p-6">
        <h3 className="pb-3 text-[16px] font-black uppercase leading-[22px] text-[#333]">
          {title.map((line, i) => (
            <span key={line}>
              {line}
              {i < title.length - 1 && <br />}
        ...
        <p className="flex-1 text-[14px] leading-[22.75px] text-[#6b7280]">
        ...
        <div className="mt-auto flex items-center justify-between border-t border-[#f3f4f6] pt-4">
          <Link
            href="#"
            className="flex items-center gap-1.5 text-[12px] font-bold uppercase tracking-[0.3px] text-app-accent-blue"
          >
            XEM THÊM
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-[12px] text-[#9ca3af]">{views}</span>
          </div>
        </div>
      </div>
    </article>
...
function SidebarSearch() {
  return (
    <div className="rounded-[16px] bg-white p-6 shadow-[0px_4px_6px_-1px_rgba(0,0,0,0.1),0px_2px_4px_-2px_rgba(0,0,0,0.1)]">
      <h4 className="mb-4 flex items-center gap-2 text-[14px] font-black uppercase text-[#333]">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className="flex-1 rounded-[8px] border border-[#e5e7eb] px-4 py-2.5 text-[14px] text-[#6b7280] outline-none transition-colors placeholder:text-[#9ca3af] focus:border-app-brand-teal"
        />
        <button className="flex size-8 shrink-0 items-center justify-center rounded-full bg-app-accent-blue text-white">
...
function SidebarCategories() {
  return (
    <div className="rounded-[16px] bg-white p-6 shadow-[0px_4px_6px_-1px_rgba(0,0,0,0.1),0px_2px_4px_-2px_rgba(0,0,0,0.1)]">
      <h4 className="mb-4 flex items-center gap-3 text-[14px] font-black uppercase text-[#333]">
        <div className="h-0.5 w-6 bg-app-brand-teal" />
      ...
      <div className="flex flex-col gap-2">
        {sidebarContent.categories.items.map((item) => (
          <button
            key={item.label}
            className={`flex items-center gap-3 rounded-[8px] px-3 py-2.5 text-left text-[14px] transition-colors ${item.highlighted ? "bg-app-brand-teal/10 font-bold text-app-accent-blue" : "text-[#6b7280] hover:bg-gray-50"}`}
          >
            <CategoryIcon icon={item.icon} />
            <span className="flex-1">{item.label}</span>
            <span
              className={`rounded-full px-2.5 py-0.5 text-[12px] ${item.highlighted ? "bg-app-accent-blue text-white" : "bg-[#f3f4f6] text-[#9ca3af]"}`}
            >
              {item.count}
            </span>
          </button>
...
function SidebarTrendingItem({
  ...
  return (
    <Link href="#" className="group flex gap-3">
      <div className="size-[72px] shrink-0 overflow-clip rounded-[8px]">
        <Image
          src={image}
          alt={alt}
          width={72}
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-center gap-1">
        <span
          className="inline-block w-fit rounded px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-[0.5px] text-white"
          style={{ backgroundColor: categoryBg }}
        >
          {category}
        </span>
        <h5 className="text-[13px] font-bold leading-[18px] text-[#333] transition-colors group-hover:text-app-accent-blue">
          {title.map((line, i) => (
            <span key={line}>
              {line}
              {i < title.length - 1 && <br />}
...
function SidebarTrending() {
  return (
    <div className="rounded-[16px] bg-white p-6 shadow-[0px_4px_6px_-1px_rgba(0,0,0,0.1),0px_2px_4px_-2px_rgba(0,0,0,0.1)]">
      <h4 className="mb-5 flex items-center gap-3 text-[14px] font-black uppercase text-[#333]">
        <div className="h-0.5 w-6 bg-app-brand-teal" />
      ...
      <div className="flex flex-col gap-5">
        {sidebarContent.trending.items.map((item, i) => (
          <div key={i}>
            ...
            {i < sidebarContent.trending.items.length - 1 && <div className="mt-5 border-t border-[#f3f4f6]" />}
          </div>
        ))}
      </div>
    </div>
...
function SidebarContactCta() {
  return (
    <div className="relative overflow-clip rounded-[16px] bg-gradient-to-br from-[#2973b2] to-[#1a4f8a] p-6 shadow-[0px_4px_6px_-1px_rgba(0,0,0,0.1),0px_2px_4px_-2px_rgba(0,0,0,0.1)]">
      <div
      ...
      />
      <div className="relative z-10 mb-4 flex size-12 items-center justify-center rounded-full bg-white/20">
      ...
      <h4 className="relative z-10 mb-2 text-[16px] font-black uppercase leading-[22px] text-white">
        {sidebarContent.contactCta.heading}
      </h4>
      <p className="relative z-10 mb-5 text-[12px] leading-[18px] text-[#bfdbfe]">
      ...
      <Link
        href="tel:0983570760"
        className="relative z-10 flex items-center justify-center gap-2 w-full rounded-[8px] bg-app-brand-teal px-4 py-3 text-[14px] font-bold text-white transition-opacity hover:opacity-90"
      >
        <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
...
export default function NewsGridSidebarSection() {
  return (
    <section className="bg-[#f8f9fa] pb-20 pt-10">
      <div className="mx-auto w-full max-w-[1280px] px-4 sm:px-6 xl:px-8">
        <div className="flex flex-col gap-10 lg:flex-row">
          <div className="min-w-px flex-1">
            <AppScrollReveal variant="fade-in-up">
              <div className="mb-8 flex items-center justify-between">
                ...
                <span className="text-[12px] font-medium text-[#9ca3af]">{newsGridContent.header.showing}</span>
              </div>
            </AppScrollReveal>
            ...
            <AppScrollReveal variant="fade-in-up" delayMs={100}>
              <div className="flex justify-center pt-8">
                <button className="flex items-center gap-2 rounded-[8px] border-2 border-app-accent-blue px-[34px] py-4 text-[14px] font-bold uppercase text-app-accent-blue transition-colors hover:bg-app-accent-blue hover:text-white">
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
          ...
          <aside className="flex w-full flex-col gap-8 lg:w-[384px] lg:shrink-0">
            <AppScrollReveal variant="fade-in-up" delayMs={50}>
              <SidebarSearch />
            </AppScrollReveal>
            <AppScrollReveal variant="fade-in-up" delayMs={100}>
