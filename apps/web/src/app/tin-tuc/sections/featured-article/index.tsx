    ...
    <section className="bg-white px-5 py-16 sm:px-8 lg:px-20">
      <AppContainer>
        <AppScrollReveal variant="fade-in-up">
          <AppSectionBadge centered className="pb-10">
            {featuredArticleContent.badge}
          </AppSectionBadge>
        </AppScrollReveal>

        <AppScrollReveal variant="fade-in-up" delayMs={100}>
          <div className="grid grid-cols-1 overflow-clip rounded-[16px] border border-[#f3f4f6] bg-white shadow-[0px_20px_25px_-5px_rgba(0,0,0,0.1),0px_8px_10px_-6px_rgba(0,0,0,0.1)] lg:grid-cols-5">
            <div className="relative lg:col-span-3">
              <div className="relative h-[300px] overflow-clip sm:h-[400px] lg:h-full lg:min-h-[728px]">
                <Image
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-r from-transparent to-[rgba(0,0,0,0.2)]" />
              </div>

              <div className="absolute left-4 top-4 flex gap-2">
                <div className="flex items-center gap-1 rounded-[6px] bg-app-brand-teal px-3 py-1.5 text-[12px] font-bold uppercase tracking-[1.2px] text-white shadow-[0px_10px_15px_-3px_rgba(0,0,0,0.1),0px_4px_6px_-4px_rgba(0,0,0,0.1)]">
                ...
                <div className="rounded-[6px] bg-app-accent-blue px-3 py-1.5 text-[12px] font-bold uppercase tracking-[1.2px] text-white shadow-[0px_10px_15px_-3px_rgba(0,0,0,0.1),0px_4px_6px_-4px_rgba(0,0,0,0.1)]">
                  {article.featuredBadgeSub}
                </div>
              </div>
            </div>

            <div className="relative flex flex-col justify-between p-6 lg:col-span-2 lg:p-10">
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-3">
                  <span className="text-[12px] font-bold uppercase tracking-[1.2px] text-app-brand-teal">
                    {article.category}
                  </span>
                  <div className="size-1 shrink-0 rounded-full bg-[#d1d5db]" />
                  <span className="text-[12px] text-[#9ca3af]">{article.readingTime}</span>
                </div>

                <h2 className="text-[26px] font-black uppercase leading-[32px] text-[#333] sm:text-[30px] sm:leading-[36px]">
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2 pt-1">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[12px] text-[#9ca3af]">{article.author}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <svg width="13.5" height="12" viewBox="0 0 13.5 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path
                    ...
                    <span className="text-[12px] text-[#9ca3af]">{article.views}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <svg width="10.5" height="12" viewBox="0 0 10.5 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path
                    ...
                    <span className="text-[12px] text-[#9ca3af]">{article.shares}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 flex items-center justify-between border-t border-[#f3f4f6] pt-5">
                <Link
                  href="#"
                  className="flex items-center gap-2 text-[14px] font-bold uppercase tracking-[0.35px] text-app-accent-blue"
                >
                  ĐỌC BÀI VIẾT
                  <svg width="10.5" height="12" viewBox="0 0 10.5 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                ...
                <div className="flex items-center gap-2">
                  <div className="size-8 overflow-clip rounded-full border-2 border-app-brand-teal">
                    <Image
                  ...
                  <span className="text-[12px] text-[#9ca3af]">{article.authorName}</span>
                </div>
              </div>

              <div className="absolute left-[-24px] top-1/2 hidden -translate-y-1/2 flex-col items-center gap-0 rounded-[12px] bg-app-accent-blue px-0 pb-3 pt-4 shadow-[0px_25px_50px_-12px_rgba(0,0,0,0.25)] lg:flex">
                <span className="text-[24px] font-black leading-[24px] text-white">{article.date.day}</span>
                <span className="pt-1 text-[12px] font-semibold uppercase tracking-[0.6px] text-[#bfdbfe]">
                  {article.date.month}
                </span>
                <span className="text-[12px] text-[#93c5fd]">{article.date.year}</span>
              </div>
            </div>
          </div>
        </AppScrollReveal>
