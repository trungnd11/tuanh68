...
export default function AboutTeamSection() {
  return (
    <section className={clsx("border-t border-[#e5e7eb] bg-[#f8f9fa] pb-[80px] pt-[81px]")}>
      <div className={clsx("mx-auto max-w-[1280px] px-[32px]")}>
        <div className={clsx("flex flex-col gap-[64px]")}>
          <div className={clsx("flex w-full flex-col items-center gap-[16px]")}>
            ...
            <AppScrollReveal variant="fade-in-up" delayMs={50}>
              <h2 className={clsx("text-center text-[36px] font-black uppercase text-[#333]")}>
                ...
                <span className={clsx("text-app-accent-blue")}>{aboutTeamContent.titleHighlight}</span>
              </h2>
            </AppScrollReveal>

            <AppScrollReveal variant="fade-in-up" delayMs={100}>
              <div className={clsx("max-w-[576px] text-center text-[16px] leading-[24px] text-[#6b7280]")}>
          ...
          <div className={clsx("flex gap-[32px] max-lg:flex-col")}>
            {aboutTeamContent.cards.map((card, i) => (
              <AppScrollReveal key={card.badge} variant="fade-in-up" delayMs={i * 100} className={clsx("flex flex-1")}>
                <div
                  className={clsx(
                    "relative flex h-[380px] flex-1 overflow-clip rounded-[16px]",
                ...
                >
                  ...
                  />
                  <div
                    className={clsx("absolute inset-0 bg-gradient-to-t from-black/80 via-black/20", "to-transparent")}
                  />
                  <div className={clsx("absolute bottom-0 left-0 right-0 flex flex-col gap-[4px] p-[24px]")}>
                    <span className={clsx("text-[12px] font-bold uppercase tracking-[1.2px] text-app-brand-teal")}>
                      {card.badge}
                    </span>
                    <div className={clsx("pt-[4px]")}>
                      <h3 className={clsx("text-[20px] font-bold leading-[28px] text-white")}>{card.title}</h3>
                    </div>
                    <p className={clsx("m-0 text-[14px] leading-[20px] text-[#d1d5db]")}>{card.description[0]}</p>
                  </div>
                </div>
              </AppScrollReveal>
            ))}
