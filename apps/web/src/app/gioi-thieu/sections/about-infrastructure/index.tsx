...
export default function AboutInfrastructureSection() {
  return (
    <section className="bg-[#111827] py-[80px]">
      <div className="mx-auto flex max-w-[1280px] flex-col items-center gap-[64px] px-[32px]">
        <AppScrollReveal variant="fade-in-up">
          <div className="flex flex-col items-center gap-[24px]">
            ...
            <h2 className="text-center text-[36px] font-black leading-[40px] uppercase text-white">
              {aboutInfrastructureContent.title[0]}
            </h2>

            <p className="max-w-[800px] text-center text-[16px] leading-[26px] text-[#9ca3af]">
          ...
          {aboutInfrastructureContent.cards.map((card, i) => {
            ...
            return (
              <AppScrollReveal key={card.number} variant="fade-in-up" delayMs={i * 100}>
                <div className="group relative flex min-h-[460px] flex-col overflow-hidden rounded-[12px]">
                  <Image
                    src={card.image}
                    alt={card.title}
                    fill
                  ...
                  <div className="relative z-10 flex flex-1 flex-col justify-between p-[24px]">
                    <div
                    ...
                    <div className="flex flex-col gap-[16px]">
                      <span className="text-[12px] font-semibold uppercase tracking-[1.2px] text-app-brand-teal">
                        {card.number} / {card.label}
                      </span>

                      <h3 className="text-[24px] font-black leading-[28px] text-white">{card.title}</h3>

                      <p className="text-[14px] leading-[22px] text-[#d1d5db]">{card.description}</p>

                      <div className="flex gap-[12px]">
                        {card.stats.map((stat) => (
                          <div
                            key={stat.label}
                            className="flex flex-1 flex-col gap-[4px] rounded-[8px] bg-white/10 px-[16px] py-[12px] backdrop-blur-sm"
                          >
                            <span className="text-[20px] font-black leading-[24px] text-white">
                              {stat.value}
                              {stat.unit && (
                                <span className="text-[12px] font-medium text-app-brand-teal"> {stat.unit}</span>
                              )}
                            </span>
                            <span className="text-[11px] font-medium uppercase tracking-[0.55px] text-[#9ca3af]">
                              {stat.label}
                            </span>
                          </div>
                        ))}
